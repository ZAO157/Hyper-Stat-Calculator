...
int main() {
	...
	//Sta
	iniSta = iniSta * 4 + subSta;
	double* sta = new double[totalPoints+1];
	for(int i = totalPoints ; i >= 0 ; i--) {
		sta[i] = iniSta;
	}
	for(int i = 0 ; i < 15 ; i++) {
		bool nop = true;
		int& weight = points[i];
		double value = ((double)(iniSta + 120)) / ((double)iniSta);
		iniSta += 120;
		for(int j = totalPoints ; j >= weight ; j--) {
			if(dp[j-weight] * value > dp[j]) {
				dp[j] = dp[j-weight] * value;
	...
	for(int i = 0 ; i < 15 ; i++) {
		...
		int& weight = points[i];
		for(int j = totalPoints ; j >= weight ; j--) {
			double value = (sta[j-weight] + 30) / sta[j-weight];
			if(dp[j-weight] * value > dp[j]) {
				dp[j] = dp[j-weight] * value;
				nop = false;
				res[j][0] = res[j-weight][0];
				res[j][1] = res[j-weight][1];
				res[j][2] = res[j-weight][2];
				res[j][3] = res[j-weight][3];
				res[j][4] = res[j-weight][4];
				res[j][5] = res[j-weight][5];
				res[j][6] = res[j-weight][6] + 1;
				sta[j] = sta[j-weight] + 30;
			}
			if(i == 15) break;
		}
		if(nop) break;
	...
	cout << "\n\nYou should use:\n" << 
		...
		res[totalPoints][6] << " level of sub stat\n" <<
